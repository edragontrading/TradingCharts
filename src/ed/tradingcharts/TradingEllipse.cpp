...
void ETradingEllipse::pointMoving(const QPointF &pos) {
...
EResizeHandle *ETradingEllipse::createPointResize() {
...
void ETradingEllipse::resizePointStoppedMoving(bool cancelled) {
...
double ETradingEllipse::selectTest(const QPointF &pos, bool onlySelectable, QVariant *details) const {
    ...
    result = std::min(result, ed::internal::distance(topLeft->pixelPosition(), pos));
    result = std::min(result, ed::internal::distance(bottomRight->pixelPosition(), pos));
